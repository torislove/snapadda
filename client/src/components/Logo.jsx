  ...
  return (
    ...
    >
      <style>{`
        ...
        .logo-building { transition:filter 0.4s ease,transform 0.4s ease; filter:drop-shadow(0 0 10px rgba(244,208,63,0.45)); }
        .logo-container-hover .logo-building { filter:drop-shadow(0 0 24px rgba(244,208,63,0.75)); transform:scale(1.1) rotate(-2deg); }
        .window-sparkle { animation:sparkle 1.5s infinite alternate ease-in-out; }
        @keyframes sparkle { 0%{opacity:0.3;transform:scale(0.9)} 100%{opacity:1;transform:scale(1.1)} }
      `}</style>

      <div className={hovered ? 'logo-container-hover' : ''} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', textDecoration: 'none' }}>
        <svg width={size} height={size} viewBox="0 0 400 400" className="logo-building" fill="none" xmlns="http://www.w3.org/2000/svg">
          <defs>
        ...
        {showText && (
          <div style={{ display: 'flex', alignItems: 'center', fontFamily: "'Inter',sans-serif", textDecoration: 'none' }}>
            <span className="logo-text-shimmer" style={{ fontSize: textSize, fontWeight: 900, letterSpacing: '-0.045em', textTransform: 'uppercase', userSelect: 'none', lineHeight: 1, display: 'inline-block' }}>
              SnapAdda
            </span>
          </div>
        )}
