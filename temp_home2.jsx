...
const SMART_PILLS = (t) => [
  ...
  { label: `≡ƒöÑ ${t('pills.under50l', 'Under Γé╣50L')}`, key: 'budget' },
  { label: `Γ£à ${t('pills.ready', 'Ready to Move')}`, key: 'ready' },
  { label: `≡ƒº¡ ${t('pills.east', 'East Facing')}`, key: 'east' },
  { label: `≡ƒÅ¢∩╕Å ${t('pills.crda', 'CRDA Approved')}`, key: 'crda' },
  { label: `Γ£ö∩╕Å ${t('pills.verified_only', 'Verified Only')}`, key: 'verified' },
];

const SORT_OPTIONS = (t) => [
  { label: t('sort.newest', 'Newest First'), value: 'newest' },
  { label: t('sort.price_low', 'Price: Low ΓåÆ High'), value: 'price_asc' },
  { label: t('sort.price_high', 'Price: High ΓåÆ Low'), value: 'price_desc' },
...
const BUDGET_OPTIONS = (t) => [
  ...
  { label: t('budget.under25', 'Under Γé╣25L'), value: '2500000' },
  { label: t('budget.25to50', 'Γé╣25LΓÇô50L'), value: '5000000' },
  { label: t('budget.50to1', 'Γé╣50LΓÇô1Cr'), value: '10000000' },
  { label: t('budget.1to2', 'Γé╣1CrΓÇô2Cr'), value: '20000000' },
  { label: t('budget.over2', 'Γé╣2Cr+'), value: '999999999' },
];
            ...
            >
                          ...
                          {getFuzzySuggestions(keyword).map(c => (
                            <button key={`${c.name}-${c.type}`} className="autocomplete-item" onClick={() => { setKeyword(c.name); setCityFilter(c.name); setShowAutocomplete(false); }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                                ...
                                <span className="ac-badge" style={{ fontSize: '0.65rem' }}>{c.type === 'District' ? 'District' : c.district || 'Andhra Pradesh'} ΓÇó {c.type}</span>
                              </div>
                            </button>
                          ))}
                        </>
                  ...
                  <div className="budget-card">
                    <div style={{ fontSize: '0.85rem', color: 'var(--txt-primary)', fontWeight: 700, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                      ...
                      <span style={{ color: 'var(--gold)', background: 'var(--gold-dim)', padding: '2px 8px', borderRadius: '6px', fontSize: '0.9rem', fontWeight: 800, border: '1px solid var(--gold-border)' }}>
                        {(!budget || budget === '999999999') ? 'Any Budget' : `Up to Γé╣${Number(budget) >= 10000000 ? (Number(budget)/10000000) + ' Cr' : (Number(budget)/100000) + ' Lakhs'}`}
                      </span>
                    </div>
              ...
              <div className="search-quick-chips">
                ...
                {[['Under 50L', '', '5000000'], ['50L-1Cr', '5000000', '10000000'], ['1Cr-2Cr', '10000000', '20000000'], ['2Cr+', '20000000', '']].map(([label, min, max]) => {
                  ...
                  return <button key={label} className={`quick-chip${active ? ' active' : ''}`} onClick={() => setAdvFilters(prev => active ? { ...prev, minPrice: '', maxPrice: '' } : { ...prev, minPrice: min, maxPrice: max })}>Γé╣{label}</button>;
                })}
